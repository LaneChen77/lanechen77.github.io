...
My long-term vision is to build AI systems that can work in the physical world and solve real-world problems.
...
> **Reproducibility note (July 2026).** The current BEV-VLM implementation uses Max-V1 as its backbone. I have identified a code-level bug in the Max-V1 backbone implementation used by BEV-VLM, which affects the validity of the experiments. Therefore, the results currently reported in BEV-VLM should not be treated as reliable until the issue is corrected and the experiments are rerun. Regardless of where a bug occurs within an integrated system, validating every component and the final results is the responsibility of the authors. As a co-author, I take responsibility for my part in not identifying this issue before release, and I am disclosing it here so that readers do not rely on the current numbers. Any corrected claims should be based on a fixed implementation and complete experimental reruns.
